...
void Mesh::readFEM(const char * node_file, const char * element_file) {
   Eigen::Vector3d vert;
   int id;
   std::string line;
   ...
   ifstream in_nodes(node_file);
   ifstream in_elements(element_file);

   check_error(!in_nodes, "Cannot find input nodes file.");
   check_error(!in_elements, "Cannot find input elements file.");

   while(std::getline(in_nodes, line)) {
      std::istringstream iss(line);
      iss >> ws;  //extract whitespaces
      if (iss.peek() == '#') {
	      continue;
      }
      iss >> vert.x();
   ...
   while(std::getline(in_elements, line)) {
      std::istringstream iss(line);
      iss >> ws;  //extract whitespaces
      if (iss.peek() == '#') {
	      continue;
      ...
      TempFace tempFace;
      while (iss >> id) {
         check_error(id > numVertices(), "Problem with input elements file: node index is out of range");
         tempFace.verts.push_back(id-1);
      }
      tempFace.ID = tempFaces.size();
      tempFaces.push_back(tempFace);
